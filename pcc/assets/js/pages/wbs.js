...
window.PAGE = (function() {
  ...
  // ── State ──
  let nodes      = [];   // [{wbsCode, wbsName, level}]  — no stale; WBS codes are backend-generated
  ...
  async function load() {
    ...
    try {
      ...
      // z12Loaded = Z12 sheet fetched and has at least one row.
      // If false (sheet not shared, wrong tab name, etc.) we skip ALL stale
      // checks so existing data is never incorrectly flagged as broken.
      const z12Loaded = validNatures.size > 0;

      // ── Load saved per-project WBS nodes ──
      // WBS Codes are auto-generated by the backend (AppSheet / Apps Script).
      // The portal reads and displays them — no stale check, no validation.
      nodes = (w || [])
        ...
        .map(r => {
          ...
          return {
            ...
            // _stale intentionally omitted — WBS rows from backend are always valid
          };
        })
        .filter(n => n.wbsCode);
      ...
      activities = (a || [])
        ...
        .map(r => {
          ...
          // Only mark stale when Z12 loaded AND values don't match — never stale on load failure
          const validTypesForNat = (z12Loaded && nature) ? (natureMap[nature] || []) : [];
          const stale = z12Loaded && (
            (!!nature && !validNatures.has(nature)) ||
            (!!type && !!nature && validTypesForNat.length > 0 && !validTypesForNat.includes(type))
          );

          return {
            name:         m ? (m['Activity'] || '') : (r['Activity'] || r['Activity Name'] || r['Name'] || ''),
            wbsCode:      r['WBS Code'] || '',     // keep saved WBS Code (not overridden by master)
            costCode:     r['Cost Code'] || r['CostCode'] || '',
            unit:         m ? (m['Unit'] || '') : (r['Unit'] || ''),
      ...
      if (!z12Loaded) {
        // Z12 didn't load — tell the user why and don't mark anything stale
        setStatus(`Z12 not loaded — Nature/Type dropdowns unavailable`, 'gold');
        Utils.toast('Z12 master (M12_Nature of Work) could not be fetched — check sheet sharing', 'err');
      } else if (staleActs) {
        setStatus(`⚠ ${staleActs} activit${staleActs===1?'y':'ies'} have unknown Nature/Type`, 'red');
        Utils.toast(`${staleActs} activit${staleActs===1?'y':'ies'} have Nature/Type not in Z12 — re-pick from master to fix`, 'err');
      } else {
        setStatus(`${nodes.length} WBS · ${activities.length} activit${activities.length===1?'y':'ies'} · Z12 OK`, 'green');
      }
    } catch (e) {
      console.error(e);
  ...
  function confirmWbsPick() {
    ...
    _wbsPickerSelected.forEach(wbsCode => {
      ...
      nodes.push({ wbsCode, wbsName: '', level: (wbsCode.match(/\./g)||[]).length });
      added++;
    });
    closeWbsPicker();
    renderTree();
