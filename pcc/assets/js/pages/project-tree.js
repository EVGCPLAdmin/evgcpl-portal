...
window.PAGE = (function () {
  ...
  function renderTree(filter) {
    ...
    const boqNodes = boqRows.map((r, i) => ({
      ...
      sNo:       v(r,'BOQ Item #')         || v(r,'S No') || String(i+1),
      desc:      v(r,'Description')        || '',
      unit:      v(r,'Unit')               || '',
      qty:       v(r,'Qty')                || '',
      rate:      v(r,'Rate')               || '',
      amount:    v(r,'Amount')             || '',
      boqId:     v(r,'BOQ ID')             || (v(r,'UUID') + '-' + (v(r,'BOQ Item #') || v(r,'S No') || String(i+1))),
      boqIdDesc: v(r,'BOQ ID (Description)') || '',
      _raw: r,
    }));
  ...
  function renderBOQLevel(boqNodes, wbsByBoq, actsByWbs, wbsOrphan, actOrphan) {
    ...
    boqNodes.forEach(b => {
      ...
      window._PCC_TREE_CACHE[ckAddWbs]  = { mode:'add',  boqUuid: b.uuid, boqDesc: b.desc, boqId: b.boqId, boqIdDesc: b.boqIdDesc };
  ...
  // ── WBS Form (PL12_WBS schema) ────────────────────────────────
  // Description = user input (Required)
  // Unit = adaptive dropdown
  // Qty = decimal
  // WBS Code = NEVER shown (protected, not written by PCC)
  function renderWBSForm(ctx, isEdit) {
    return `
    <div class="drawer-form" id="drawerForm" data-mode="wbs">
      <div class="form-note info">
        🌳 WBS work package under BOQ:
        <strong>${Utils.esc(ctx.boqDesc||ctx.boqId||ctx.boqUuid||'—')}</strong>
      </div>

      <div class="field">
        <label>Description <span class="req">*</span></label>
        <input type="text" id="df_wbsDesc"
               value="${Utils.esc(ctx.wbsName||ctx.desc||'')}"
               placeholder="Work scope description…" />
      </div>
      <div class="form-grid cols-2">
        <div class="field">
          <label>Unit</label>
          <input type="text" id="df_wbsUnit" list="wbsDrawerUnits"
                 value="${Utils.esc(ctx.unit||'')}"
                 placeholder="Running Meter, CUM…"
                 autocomplete="off" />
          <datalist id="wbsDrawerUnits">
            <option value="Running Meter"><option value="Kilogram">
            <option value="Numbers"><option value="Square Meter">
            <option value="Cubic Meter"><option value="Metric Ton">
            <option value="Lump Sum">
          </datalist>
        </div>
        <div class="field">
          <label>Qty</label>
          <input type="number" id="df_wbsQty"
                 value="${ctx.qty||''}"
      ...
      <input type="hidden" id="df_boqId"      value="${ctx.boqId||''}" />
      <input type="hidden" id="df_boqIdDesc"  value="${Utils.esc(ctx.boqIdDesc||'')}" />
    </div>`;
  }
  ...
  async function _saveWBSFromDrawer() {
    const ap      = window.STATE.activeProject;
    const desc    = document.getElementById('df_wbsDesc')?.value   || '';
    const unit    = document.getElementById('df_wbsUnit')?.value   || '';
    const qty     = Number(document.getElementById('df_wbsQty')?.value || 0);
    const uuid    = document.getElementById('df_uuid')?.value      || '';
    const boqUuid = document.getElementById('df_boqUuid')?.value   || '';
    const boqId   = document.getElementById('df_boqId')?.value     || '';
    const boqIdDesc = document.getElementById('df_boqIdDesc')?.value || '';

    if (!desc.trim()) { Utils.toast('Description is required', 'err'); return; }

    // Collect all existing WBS rows + add/replace this one
    const allNodes = wbsRows.map((r, i) => ({
      uuid:        v(r,'UUID')       || '',
      checkSum:    v(r,'CheckSum')   || '',
      boqId:       v(r,'BOQ ID')     || '',
      boqIdDesc:   v(r,'BOQ ID (Description)') || '',
      description: v(r,'Description') || v(r,'WBS Name') || '',
      unit:        v(r,'Unit')       || '',
      qty:         Number(v(r,'Qty')) || 0,
    }));

    const thisNode = {
      uuid, checkSum: boqUuid, boqId, boqIdDesc,
