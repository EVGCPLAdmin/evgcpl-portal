...
// Keep only the newest batch's rows (by BatchId, which sorts chronologically).
function _tvrLatestBatch(tab) {
  ...
  var mine = rows.filter(function (r) { return String(r['BatchId'] || '') === newest; });
  return {
    batchId: newest,
...
function tvrRunNow(body) {
  body = body || {};
  return runDailyVendorReconcile({ skipEmail: !!body.skipEmail });
}
...
function tvrGetStatus(body) {
  ...
  // Latest reconciliation run only — older runs stay in the tab as history.
  var all = _tvrRows(TVR_TAB_MISMATCH);
  var newestRun = '';
  all.forEach(function (r) { var id = String(r['RunId'] || ''); if (id > newestRun) newestRun = id; });
  var current = all.filter(function (r) { return String(r['RunId'] || '') === newestRun; });
